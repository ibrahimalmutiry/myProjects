...
function createInternalNotification($type, $scope, $refId, $refNumber, $title, $body, $severity, $recipientId = null) {
    ...
    $txId        = $refId ? (int)$refId : 'NULL';
    $title       = $conn->real_escape_string($title);
    $message     = $conn->real_escape_string($body ?? $title);
    $employeeId  = $recipientId ? (int)$recipientId : 'NULL';

    $conn->query("INSERT INTO system_notifications
                  (type, category, title, message, transaction_id, employee_id)
                  VALUES ('$dbType','$category','$title','$message',$txId,$employeeId)");
...
function getSystemNotifications($userId = null, $limit = 50, $unreadOnly = false) {
    ...
    $where = $uid > 0
        ...
        : "1=1";
    if ($unreadOnly) $where .= " AND sn.is_read = 0";

    $res  = $conn->query("SELECT sn.*,
...
/**
 * جلب إشعارات SLA/OLA المخصصة لموظف معين
 * تُستخدم في endpoint: notifications
 */
function getSlaNotificationsForUser($userId, $limit = 50) {
    ...
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            // إضافة transaction_type من الـ JOIN إن وُجد
            if (empty($r['transaction_type']) && !empty($r['transaction_id'])) {
                $ttRes = $conn->query("SELECT tt.name FROM transactions tx
                                       LEFT JOIN transaction_types tt ON tx.type_id = tt.id
                                       WHERE tx.id = " . (int)$r['transaction_id'] . " LIMIT 1");
                if ($ttRes && $ttRes->num_rows > 0) {
                    $r['transaction_type'] = $ttRes->fetch_assoc()['name'] ?? '';
                }
            }
            $rows[] = $r;
        }
...
function getUnreadNotificationCount($userId = null) {
    $conn  = db();
    $uid   = $userId ? (int)$userId : 0;
    $where = $uid > 0 ? "(employee_id = $uid OR employee_id IS NULL)" : "1=1";
