...
<head>
    ...
    <title>تسجيل الدخول | نظام إدارة المعاملات</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .employee-card.show { display: flex; }
        ...
        @media (max-width: 480px) {
            ...
            .features { flex-direction: column; gap: 0.75rem; align-items: center; }
...
<body>
        ...
        <div class="login-card">
            ...
            <form id="loginForm">
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="employeeNumber" placeholder="أدخل رقمك الوظيفي" autocomplete="off" required>
                </div>
    ...
    document.getElementById('loginForm').addEventListener('submit', async function(e) {
        ...
        if (!empNumber) { showMessage('الرجاء إدخال الرقم الوظيفي', 'error'); return; }
            ...
            if (step === 'check') {
                ...
                if (data.success) {
                    ...
                    if (data.data.is_registered == 1) {
                        ...
                        setTimeout(() => { window.location.href = 'register.php?emp=' + encodeURIComponent(empNumber); }, 1500);
                        return;
                    }
                } else {
                ...
                if (!password) { showMessage('الرجاء إدخال كلمة المرور', 'error'); btn.disabled = false; loader.style.display = 'none'; return; }
                
                const res = await fetch('api/auth.php?action=login', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ employee_number: empNumber, password: password })
                });
                const data = await res.json();
        ...
        } catch (e) { showMessage('حدث خطأ في الاتصال', 'error'); }
    ...
    function getRoleName(role) {
        return { 'admin': 'مدير النظام', 'receiver': 'قسم الاستلام', 'budget': 'قسم الموازنة', 'payment': 'قسم الدفع', 'invoice': 'قسم الفوترة' }[role] || role;
    }
    </script>
</body>
</html>
